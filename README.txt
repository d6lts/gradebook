...
1.  Gradebook Attachments allows teachers and/or users to attach files to grades.
    ...
    F. Restrictions can be placed on student attachments. The default behavior for an individual gradebook
       is set through the settings tab on the gradebook page. Teachers can override this default behavior
       by setting the attachments restrictions on the node edit page when creating assignments. Options
       include: do not permit attachments after the due date, do not permit attachments after a grade has
       been given, and do not permit attachments (at all).
